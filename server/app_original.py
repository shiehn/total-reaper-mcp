...
async def main_async():
    ...
    if relay_url and auth_token:
        logger.info(f"WebSocket relay enabled: {relay_url}")
        logger.info("Starting relay connection...")
        
        # Create relay client
        global relay_client
        relay_client = await create_relay_client(mcp)
        
        if relay_client:
            logger.info("WebSocket relay client started")
        else:
            logger.warning("Failed to create relay client")
    else:
        logger.info("WebSocket relay disabled (set MCP_RELAY_URL and MCP_AUTH_TOKEN to enable)")
    ...
    # Run the server
    # Note: In the WebSocket mode, we still run stdio for local connections
    # but also accept commands via WebSocket
